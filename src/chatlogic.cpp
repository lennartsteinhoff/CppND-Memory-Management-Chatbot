...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; }); //CHANGED

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            _nodes.emplace_back(std::make_unique<GraphNode>(id)); //CHANGED
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](std::unique_ptr<GraphNode> &node) { return node->GetID() == std::stoi(parentToken->second); }); //CHANGED
    ...
    // add chatbot to graph root node
  	...
  	ChatBot chatBot = ChatBot("../images/chatbot.png"); // CHANGED TASK5
    chatBot.SetRootNode(rootNode); // CHANGED TASK5
  	chatBot.SetChatLogicHandle(this);
    rootNode->MoveChatbotHere(std::move(chatBot)); // CHANGED TASK5
